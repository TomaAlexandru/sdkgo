 ...
 * @param "Depth" (optional.) -  Controls the details depth of response objects.  Eg. GET /datacenters/[ID]  - depth=0: only direct properties are included. Children (servers etc.) are not included  - depth=1: direct properties and children references are included  - depth=2: direct properties and children properties are included  - depth=3: direct properties and children properties and children's children are included  - depth=... and so on
 * @param "XContractNumber" (optional.) -  Users having more than 1 contract need to provide contract number, against which all API requests should be executed
 * @param "FilterStatus" (optional.) -  Request status filter to fetch all the request based on a particular status [QUEUED, RUNNING, DONE, FAILED]
 * @param "FilterCreatedAfter" (optional.) -  Filter all the requests after the created date
 * @param "FilterCreatedBefore" (optional.) -  Filter all the requests before the created date
 * @param "FilterUrl" (optional.) -  Url filter
 * @param "FilterCreatedDate" (optional.) -  Created date filter
 * @param "FilterMethod" (optional.) -  Method filter
 * @param "FilterBody" (optional.) -  Url filter
@return Requests
*/
func (a *RequestApiService) RequestsGet(ctx _context.Context, optionals *RequestsGetOpts) (Requests, *APIResponse, error) {
	...
	if optionals != nil && optionals.FilterStatus != nil {
		localVarQueryParams.Add("filter.status", parameterToString(*optionals.FilterStatus, ""))
	}
	if optionals != nil && optionals.FilterCreatedAfter != nil {
		localVarQueryParams.Add("filter.createdAfter", parameterToString(*optionals.FilterCreatedAfter, ""))
	}
	if optionals != nil && optionals.FilterCreatedBefore != nil {
		localVarQueryParams.Add("filter.createdBefore", parameterToString(*optionals.FilterCreatedBefore, ""))
	}
	if optionals != nil && optionals.FilterUrl != nil {
		localVarQueryParams.Add("filter.url", parameterToString(*optionals.FilterUrl, ""))
	}
	if optionals != nil && optionals.FilterCreatedDate != nil {
		localVarQueryParams.Add("filter.createdDate", parameterToString(*optionals.FilterCreatedDate, ""))
	}
	if optionals != nil && optionals.FilterMethod != nil {
		localVarQueryParams.Add("filter.method", parameterToString(*optionals.FilterMethod, ""))
	}
	if optionals != nil && optionals.FilterBody != nil {
		localVarQueryParams.Add("filter.body", parameterToString(*optionals.FilterBody, ""))
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}
